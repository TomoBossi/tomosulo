...
function setup() {
  ...
  s = windowWidth/467; // Eyeballed (praise the lord :praying-hands-emoji:)
...
function drawTitleCard() {
  ...
  text('v0.0.0.0.2 - Cannot be trusted?', rsALU.pos.x + buttonWidth + tableSeparation + 3*s, tablePaddingTop + titleTextSize + 3*s);
  textSize(versionTextSize*0.8);
  const yOffset = 9*s;
  text('Features:', rsALU.pos.x + buttonWidth + tableSeparation + 3*s, tablePaddingTop + titleTextSize + yOffset);
  text('- Writable Memory (REAL unsafety for REAL programmers)', rsALU.pos.x + buttonWidth + tableSeparation + 3*s, tablePaddingTop + titleTextSize + yOffset + 5*s);
  text('- 16 whole 16-bit Registers', rsALU.pos.x + buttonWidth + tableSeparation + 3*s, tablePaddingTop + titleTextSize + yOffset + 9.5*s);
  text('- Supports ADD, SUB, MUL, AND, OR, MOV, LDR, STR (kind of)', rsALU.pos.x + buttonWidth + tableSeparation + 3*s, tablePaddingTop + titleTextSize + yOffset + 14*s);
  text('- Accessing Memory auto-generates LDR or STR instructions', rsALU.pos.x + buttonWidth + tableSeparation + 3*s, tablePaddingTop + titleTextSize + yOffset + 18.5*s);
}
  ...
  rsLSU = drawTable(
    {
      ...
      highlightPersistent: Array(numInstructions-2).fill(0),
      outputValue: {tag: '', value: 0},
      outputLabel: 'Load-Store' // 'Load-Store Unit'
    }, true, true
  );
...
function formatInstructionDisplay(instruction) {
  try {
    ...
    let secondTokenIsRegisterOrStore = isRegister(tokens[1]) || tokens[1][0] === '[';
    let allArgsAreRegistersOrValues = areRegistersOrValues(tokens.slice(2));
    if (instructionSupported && matchingNumArgs && secondTokenIsRegisterOrStore && allArgsAreRegistersOrValues) {
